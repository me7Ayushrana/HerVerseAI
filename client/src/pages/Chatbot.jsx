...
`;
  ...
  const runFallback = (userText, chatHistory, prefix = "") => {
    ...
    } else if (msgLower.includes('online') || msgLower.includes('active') || msgLower.includes('offline') || msgLower.includes('are you there') || msgLower.includes('status')) {
      reply = `HerVerse AI Status Check\n\nI am currently running in **Local Guide Mode** because the connected Gemini API key is offline or has exceeded its rate limit.\n\nWhile running offline:\n1. All core trackers (period tracking, water logs, mental wellness diary, and fitness logs) are fully functional.\n2. The chatbot responds directly using local expert knowledge presets.\n3. The Nutrition & Meal Planner uses a balanced, cycle-synced offline recipe engine.\n\nYou can easily activate live Gemini AI answers by generating a new API key from Google AI Studio and adding it in our Settings Drawer (the gear icon at the top right of this screen)!`;
